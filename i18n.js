...
const T = {
  ...
  en: {
    ...
    'p.seo.promo': '+ SEO — full base setup that already brings in clients',
    'p.seo.promo.price': 'CHF 2 500',
    'p.geo.setup': '+ GEO — Generative Engine Optimization (visibility in AI answers)',
    'p.geo.setup.price': 'CHF 1 000',
    ...
    'faq.q8': 'On what platform / with what technologies is the site built?',
    'faq.a8': '<p>All sites are built on <b>real code, not on website builders</b> (Wix, Squarespace, Tilda and the like). Under the hood — standard, future-proof web technologies: HTML5, CSS3, JavaScript, Tailwind CSS for premium projects, and React for dynamic interfaces and web apps. For projects more complex than a landing — Supabase (PostgreSQL database, authentication, forms, reviews). For AI features — Claude API or GPT API.</p><p>This means the site belongs entirely to you — with all source code and assets. <b>You\'re not locked into a platform or a single developer</b> — in the future, any specialist can pick it up. No mandatory monthly payments for a builder. The site is fast because the code is written specifically for your task.</p><p>After delivery I hand over all source code. On request, I can also upload it to your GitHub repository.</p>',
    'faq.q9': 'How can I make changes later? Is there support?',
    'faq.a9': '<p><b>Yes, support is available in both cases</b> — whether your site is hosted on your own server or on my Swiss hosting.</p><p>If you need to:</p><ul><li>edit texts or images</li><li>add a new section or page</li><li>connect a new service</li><li>set up analytics or email campaigns</li><li>fix something or extend a feature</li></ul><p>— just write to me. I work <b>on request, without a mandatory monthly subscription</b>. Cost of each support task is estimated per scope.</p>',
  ...
  de: {
    ...
    'p.seo.promo': '+ SEO — vollständige Basis-Einrichtung, die bereits Kunden bringt',
    'p.seo.promo.price': 'CHF 2 500',
    'p.geo.setup': '+ GEO — Generative Engine Optimization (Sichtbarkeit in KI-Antworten)',
    'p.geo.setup.price': 'CHF 1 000',
    ...
    'faq.q8': 'Auf welcher Plattform / mit welchen Technologien wird die Website gebaut?',
    'faq.a8': '<p>Alle Websites werden auf <b>echtem Code gebaut, nicht auf Website-Baukästen</b> (Wix, Squarespace, Jimdo und Ähnlichem). Unter der Haube — standardisierte, zukunftssichere Webtechnologien: HTML5, CSS3, JavaScript, Tailwind CSS für Premium-Projekte und React für dynamische Interfaces und Webapplikationen. Für komplexere Projekte als eine Landingpage — Supabase (PostgreSQL-Datenbank, Authentifizierung, Formulare, Bewertungen). Für KI-Funktionen — Claude API oder GPT API.</p><p>Das bedeutet: die Website gehört vollständig Ihnen — mit allem Quellcode und allen Assets. <b>Sie sind weder an eine Plattform noch an eine einzelne Entwicklerin gebunden</b> — in Zukunft kann jede Fachperson daran weiterarbeiten. Keine verpflichtenden monatlichen Baukasten-Gebühren. Die Seite ist schnell, weil der Code speziell für Ihre Aufgabe geschrieben ist.</p><p>Nach Abschluss übergebe ich Ihnen den gesamten Quellcode. Auf Wunsch lade ich ihn zusätzlich in Ihr GitHub-Repository hoch.</p>',
    'faq.q9': 'Wie kann ich später Änderungen vornehmen? Gibt es Support?',
    'faq.a9': '<p><b>Ja, Support ist in beiden Fällen verfügbar</b> — egal ob die Site auf Ihrem eigenen Hosting oder auf meinem Schweizer Hosting liegt.</p><p>Wenn Sie:</p><ul><li>Texte oder Bilder ändern möchten</li><li>eine neue Sektion oder Seite hinzufügen wollen</li><li>einen neuen Dienst anbinden wollen</li><li>Analytics oder E-Mail-Kampagnen einrichten möchten</li><li>etwas korrigieren oder eine Funktion erweitern wollen</li></ul><p>— schreiben Sie mir einfach. Ich arbeite <b>auf Anfrage, ohne verpflichtende monatliche Abos</b>. Die Kosten jeder Support-Aufgabe werden nach Umfang kalkuliert.</p>',
  ...
  fr: {
    ...
    'p.seo.promo': '+ SEO — configuration de base complète qui amène déjà des clients',
    'p.seo.promo.price': 'CHF 2 500',
    'p.geo.setup': '+ GEO — Generative Engine Optimization (visibilité dans les réponses IA)',
    'p.geo.setup.price': 'CHF 1 000',
    ...
    'faq.q8': 'Sur quelle plateforme / avec quelles technologies le site est-il développé ?',
    'faq.a8': '<p>Tous les sites sont développés sur <b>du vrai code, pas sur des constructeurs</b> (Wix, Squarespace, Tilda et similaires). Sous le capot — des technologies web standard et pérennes : HTML5, CSS3, JavaScript, Tailwind CSS pour les projets premium, et React pour les interfaces dynamiques et applications web. Pour les projets plus complexes qu’une landing — Supabase (base de données PostgreSQL, authentification, formulaires, avis). Pour les fonctionnalités IA — l’API Claude ou GPT.</p><p>Cela signifie que le site vous appartient entièrement — avec tout le code source et tous les fichiers. <b>Vous n’êtes liés à aucune plateforme ni à un seul prestataire</b> — à l’avenir, n’importe quel spécialiste pourra reprendre le projet. Aucun paiement mensuel obligatoire pour un constructeur. Le site est rapide parce que le code est écrit spécifiquement pour votre besoin.</p><p>Après la livraison, je vous transmets l’intégralité du code source. Sur demande, je peux aussi le déposer dans votre dépôt GitHub.</p>',
    'faq.q9': 'Comment puis-je faire des modifications plus tard ? Y a-t-il un support ?',
    'faq.a9': '<p><b>Oui, le support est disponible dans les deux cas</b> — que votre site soit hébergé sur votre propre serveur ou sur mon hébergement suisse.</p><p>Si vous avez besoin :</p><ul><li>de modifier des textes ou des images</li><li>d’ajouter une nouvelle section ou page</li><li>de connecter un nouveau service</li><li>de configurer l’analytique ou des campagnes e-mail</li><li>de corriger quelque chose ou d’étendre une fonctionnalité</li></ul><p>— écrivez-moi simplement. Je travaille <b>à la demande, sans abonnement mensuel obligatoire</b>. Le coût de chaque demande de support est calculé selon son ampleur.</p>',
  ...
  it: {
    ...
    'p.seo.promo': '+ SEO — configurazione di base completa che porta già clienti',
    'p.seo.promo.price': 'CHF 2 500',
    'p.geo.setup': '+ GEO — Generative Engine Optimization (visibilità nelle risposte AI)',
    'p.geo.setup.price': 'CHF 1 000',
    ...
    'faq.q8': 'Su quale piattaforma / con quali tecnologie viene realizzato il sito?',
    'faq.a8': '<p>Tutti i siti vengono realizzati con <b>codice reale, non con costruttori di siti</b> (Wix, Squarespace, Tilda e simili). Sotto il cofano — tecnologie web standard e a prova di futuro: HTML5, CSS3, JavaScript, Tailwind CSS per i progetti premium e React per interfacce dinamiche e applicazioni web. Per progetti più complessi di una landing — Supabase (database PostgreSQL, autenticazione, form, recensioni). Per funzionalità IA — l’API Claude o GPT.</p><p>Ciò significa che il sito appartiene interamente a Lei — con tutto il codice sorgente e i materiali. <b>Non è vincolato a una piattaforma né a un singolo sviluppatore</b> — in futuro qualsiasi professionista potrà proseguire il lavoro. Nessun pagamento mensile obbligatorio per un costruttore. Il sito è veloce perché il codice è scritto specificamente per il Suo compito.</p><p>Al termine Le consegno l’intero codice sorgente. Su richiesta, posso anche caricarlo nel Suo repository GitHub.</p>',
    'faq.q9': 'Come posso fare modifiche in seguito? C’è un servizio di supporto?',
    'faq.a9': '<p><b>Sì, il supporto è disponibile in entrambi i casi</b> — sia che il sito sia ospitato sul Suo hosting che sul mio hosting svizzero.</p><p>Se Le serve:</p><ul><li>modificare testi o immagini</li><li>aggiungere una nuova sezione o pagina</li><li>collegare un nuovo servizio</li><li>impostare l’analytics o campagne e-mail</li><li>correggere qualcosa o estendere una funzionalità</li></ul><p>— mi scriva semplicemente. Lavoro <b>su richiesta, senza abbonamenti mensili obbligatori</b>. Il costo di ogni intervento di supporto è calcolato in base all’ambito.</p>',
